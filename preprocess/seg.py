...
def seg_hand_depth(img, gap=100, thresh=500, padding=10, output_size=96, scale=10, add=5, box_z=250,
                   zero_as_infty=False,
                   fore_p_thresh=300, label=None, norm=True):
    ...
    if label is not None:
        label = label.astype(np.float32)
        label[:, 0] = label[:, 0] - y_min - 170
        label[:, 1] = label[:, 1] - x_min - 100
        ...
        # return output, rgb
        # save crop_uv x is v direction, y is u direction
        return output, np.array([x_max, x_min, y_max, y_min])
