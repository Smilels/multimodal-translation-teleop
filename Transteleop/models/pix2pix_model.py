...
#
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD = networks.define_D(D_output_nc, opt.ndf, opt.netD,
    ...
    def forward(self):
        ...
        if self.opt.stn:
            self.fake_B, self.stn_A = self.netG(self.real_A)  # G(A)
        else:
    ...
    def backward_D(self):
        ...
        if self.opt.conditional_D:
            ...
            if self.opt.stn:
                fake_AB = torch.cat((self.stn_A, self.fake_B), 1)
            else:
        ...
        if self.opt.conditional_D:
            if self.opt.stn:
                real_AB = torch.cat((self.stn_A, self.real_B), 1)
            else:
